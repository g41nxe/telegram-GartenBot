...
Diese Regel beschränkt die **Abhängigkeitsrichtung** (kein Adapter→Adapter-Import), nicht den Ort von Code: typisierte Persistenz-Zugriffe leben deshalb *im* Persistenz-Adapter `database.py`, nicht in einem separaten Zustands-Modul, das `database` importieren müsste (ADR 0045).
...
Diese Regel verbietet `core → adapters`-**Importe**, nicht reinen Code außerhalb von `core`: reine Funktionen, die an ein externes Datenformat oder einen konkreten Adapter gekoppelt sind (z. B. die Open-Meteo-Aggregationen), bleiben beim Adapter. Der Ort folgt der Kopplung, nicht der Reinheit (ADR 0045).
