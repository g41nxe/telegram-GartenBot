...
* **Tabelle `device_status_log`**: Um die Spalte `device_name` (TEXT) erweitern, um Statuslogs (Battery, LQI) pro Ventil separat zu speichern.
* **Migrationen**:
  * Beim Start prüft `init_db()` das Schema. Wenn die Tabelle `valves` fehlt, erstellen wir sie und legen ein Standard-Eintrag (`id=1`, `name="garden_valve"`, `mqtt_name="garden_valve"`, `is_paired=1`) an.
  * Bestehende Zeitpläne in `schedules` werden über `schedule_valves` mit dem Standard-Ventil (`valve_id=1`) verknüpft.
  * Bestehende Einträge in `watering_history` und `device_status_log` werden auf `valve_id=1` bzw. `device_name="garden_valve"` gesetzt.
...
* Beim Verbindungsaufbau liest der Client alle registrierten Ventile aus der Datenbank und abonniert deren individuelle Topics.
* **Dynamische Topic-Struktur:** Alle Ventil-Topics werden dynamisch nach dem Muster `zigbee2mqtt/{mqtt_name}` aufgelöst (kein statischer Rückgriff auf `config.MQTT_VALVE_TOPIC` für Einzelventile).
* Die API `open_valve(mqtt_name)` und `close_valve(mqtt_name)` nimmt nun den Topic-Namen des Ventils entgegen.
* Das Event `ValveStatusReported` erhält ein zusätzliches Argument `mqtt_name: str` zur eindeutigen Identifikation des Senders im Event-Handling.
...
* Der `WateringController` verwaltet ein Dictionary aktiver Guss-Zyklen `_active_cycles` (indiziert nach `valve_id`).
...
* Domain-Events (`WateringCycleStarted`, `WateringCycleCompleted`, etc.) werden um ein `valve_id` Feld ergänzt, damit Benachrichtigungen ventilgenau erfolgen.
...
* Der tägliche Statusbericht fragt vorab den Status aller Ventile einzeln ab (durch Publish auf das jeweilige `/get` Topic) und listet LQI, Batteriestände und Fehlermeldungen einzeln auf.
...
* **Kopplungs-Assistent (`/setup`)**: Fragt vor der Kopplung nach dem Wunschnamen des Ventils, startet dann die Suche und registriert das Ventil mit dem MQTT-Namen `valve_<ieee_address>` in der Datenbank.
* **Zeitplan-Assistent (`/zeitplan`)**:
  * Neue Multi-Select UI zur Auswahl der Ventile mittels Inline-Tastatur mit Ankreuz-Symbolen und einem "Done"-Bestätigungsknopf. *Wird übersprungen, falls nur 1 Ventil existiert.*
  * Auswahl des Ausführungsmodus (sequentiell/parallel), falls mehr als ein Ventil gewählt wurde. *Wird übersprungen, falls nur 1 Ventil existiert.*
* **Manueller Start (`🟢 Bewässern starten`)**: Erlaubt ebenfalls Multi-Select von Ventilen via Toggle-Keyboard sowie die Auswahl des Ausführungsmodus. *Wird übersprungen, falls nur 1 Ventil existiert (Auto-Selektion).*
